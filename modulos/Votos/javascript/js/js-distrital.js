...
var id=$("#table_distrito").jqGrid("getGridParam",'selrow');
	if (id) {
		...
		daoParticipacion.listarPartidosDistrital(HTMLlistarPartidos,id);		
    }
	...
	if(datos.length>0){
		$.each(datos,function(index,data){
			...
			htm='<tr class="elimina" id="'+data.idparticipacion+'" data-pos="'+data.nroorden+'">'+
			    	...
			    	'<td><input type="number" class="span9 input-lg" id="distrital_'+data.idparticipacion+'" placeholder="'+p1+'" min="0" onKeyPress="return validaNumeros(event,this.id,4);" autofocus required '+d1+'></td>'+
			    '</tr>';
...
GuardarVotos=function(){
	...
	$(".elimina").each(function() {
		contador++;
		if($("#distrital_"+this.id).val()=='' && error==0 && !$("#distrital_"+this.id).attr("disabled")){
			error=1;
			_msjAdvertencia('Ingrese su Voto Distrital Nro:<b>'+this.getAttribute("data-pos"))+'</b>';
			$("#distrital_"+this.id).focus();			
		}
		else{
			if(this.id!='btnguardar2'){
				if(!$("#distrital_"+this.id).attr("disabled")){
					total+=$("#distrital_"+this.id).val()*1;
					datos.push(this.id+'_'+$("#distrital_"+this.id).val());
				}				
			}			
		}		
	});	
	...
	if(error==0 && contador>0){
		$(".escondefinal").css("display","none");
		daoParticipacion.guardarRegistrosDistrital(datos.join("|"),1);		
	}
}
...
GuardarFinal=function(){
	...
	if(error==0 && contador>0){
		$(".escondefinal").css("display","none");
		daoParticipacion.guardarRegistrosDistrital(datos.join("|"),0);		
	}
}
